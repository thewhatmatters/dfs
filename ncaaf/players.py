"""Load a FanDuel NCAAF players-list CSV into a flat player pool."""
...
class Player:
    ...
    # Rank 1 = OurLads starter at that alignment (role prior, not snap %).
    ...
    # props | no_market | unmatched — set when Odds props are attached.
    prop_status: str | None = None
    # Team play mix (CFBD advanced). None until 2026 boxes exist.
    pass_rate: float | None = None
    opp_pass_rate: float | None = None
    # Opponent D PPA allowed (CFBD). Higher = worse on that play type.
    opp_rush_ppa: float | None = None
    opp_pass_ppa: float | None = None
    # Hooks only — unused until those pipelines exist. Do not invent usage.
    ownership: float | None = None
    weather: str | None = None
    rush_share: float | None = None
    target_share: float | None = None
    # False when this run ignored blowout sit (script_mult forced to 1.0).
    script_applied: bool = True
    ...
    *,
    ...
    for pl in players:
        ...
        if drop_questionable and pl.injury in questionable_codes:
            continue
        if require_fppg and not include_unprojected and pl.fppg is None:
            continue
