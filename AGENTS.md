...
NFL (`/optimize-nfl-classic`): no interview / question card. Newest `nfl/data/FanDuel-NFL-*-players-list.csv`. `python3 -m nfl.optimize --csv … --sim` (mean default). GPP / ceiling → `--objective=ceiling`. `n-lineups` only if the user named a count. Cached props; do not `--refresh-props`. Uploads: `nfl/export/` when n>1. `--bring-back` default 0. House max **3** per team (`--max-per-team=4` restores FanDuel lobby 4). `--stack-qb` on (2+ WR/TE from a team ⇒ that team's QB; `--stack-qb=off` disables).
