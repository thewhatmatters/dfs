...
`nfl/lines.py`, `nfl/injuries.py`, `nfl/depth.py`, `nfl/props.py`.

On failure, report the **id** (e.g. `LINES_KEY`, `INJ_ESPN`, `DEPTH_ESPN`,
`PROPS_ODDS_KEY`) and the stop-vs-degrade row. Green runs do not dump the
catalog. Do not scrape FanDuel.
