...
`--population pregame` (default) counts depth-chart starters even when the box score is zero or missing. A missing stats row scores 0 fantasy points and stays in the pool unless that week's injuries list them Out or IR before kickoff: `date_modified` must be earlier than kickoff when the field exists (2024); otherwise the weekly status is pre-game (2025). Actuals match `gsis_id` first and the player name second. The text report prints `id-matched`, `name-matched`, `kept-as-zero`, and `excluded-as-Out`. `--population played` restores the previous starter filter and does not invent a zero for a missing row. A pool row whose draws are not length `n` (the same pid on two chart rows) is skipped, counted, and named; the depth pool is not deduped. Monte Carlo SE and CI columns print four decimal places. The default report does not import NumPy or SciPy, so a machine without those libraries still prints the tables above.
