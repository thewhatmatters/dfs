...
`--population pregame` (default) counts depth-chart starters even when every counting stat is zero. `--population played` restores the previous starter filter. The default report does not import NumPy or SciPy, so a machine without those libraries still prints the tables above.
