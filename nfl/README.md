...
JSON on stdout; picker (QB, RB, RB, WR, WR, WR, TE, FLEX, DEF) on stderr — last column is **Sources** (join tags). Slate coverage: `python3 -m nfl.status --csv …` or `--slate-status` (JSON `slate_status`) — **relevant-band** first (QB>$6500, WR>$5000, RB>$5000, TE>$4500, DEF>$3500), full pool on the `all pool` line. Needs `ODDS_API_KEY` for game lines (hard `LINES_KEY` if missing). `--lines-source=gangstash` uses `GANGSTASH_API_KEY` instead. Player props use `GANGSTASH_API_KEY` (or a cache under `nfl/data/gangstash-props/`). Targets default to the Lineups CSV (`--targets-source=gangstash` optional). Snaps default to the Lineups CSV (`--snaps-source=gangstash` optional; `offense_pct` is the 0–1 snap share). Cached OurLads depth (`python3 -m nfl.depth --csv …`), ESPN injuries, and Gangstash props; do not `--refresh-props` unless the cache is empty. `--depth-source=espn` or `--depth-source=gangstash` are optional fallbacks.
